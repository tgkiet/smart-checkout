...
Thay vì quét mã vạch thủ công, người dùng chỉ cần tải lên một bức ảnh của nhóm sản phẩm cần thanh toán. Hệ thống sẽ lần lượt thực hiện ba bước xử lý liên tiếp: (1) **phát hiện và phân vùng** từng sản phẩm trong ảnh bằng mô hình Instance Segmentation (YOLO), tách riêng từng vật thể thành ảnh độc lập; (2) **trích xuất đặc trưng** hình ảnh của từng sản phẩm bằng mô hình Embedding (CLIP) và thực hiện tìm kiếm tương đồng so sánh với cơ sở dữ liệu vector đã được lập chỉ mục; (3) **tra cứu, tổng hợp** và trả về giỏ hàng hoàn chỉnh bao gồm tên sản phẩm, giá tiền từng mặt hàng và tổng số tiền cần thanh toán — toàn bộ trong thời gian thực.
...
Ảnh sản phẩm thu thập từ web thường gặp nhiều vấn đề chất lượng ảnh hưởng trực tiếp đến độ chính xác của vector embedding. Bốn nhóm vấn đề chính được hệ thống xác định và xử lý có hệ thống trong giai đoạn Preprocessing:
...
Trong dự án, hai phiên bản mô hình được sử dụng tùy theo ngữ cảnh: `yolov8x-seg.pt` được nạp trong Inference API server (`api_server.py`, dòng 16) — đây là phiên bản lớn nhất và có độ chính xác cao nhất trong họ YOLOv8; và `yolo11n-seg.pt` có sẵn trong repository tại `processing/yolo11n-seg.pt` — phiên bản nano nhỏ gọn, phù hợp cho môi trường Spark local mode với tài nguyên hạn chế.
...
| **Upsert** | Thao tác Insert hoặc Update — idempotent | Batch 64 points/lần (xác minh từ `data_processing.py`, dòng 198) |
...
Cấu trúc lưu trữ MongoDB trong VSSCS được phân tầng theo từng giai đoạn của pipeline, xác minh từ toàn bộ source code:
...
Cấu trúc lưu trữ MinIO trong VSSCS theo từng giai đoạn pipeline, xác minh từ source code:
...
File `kafka-docker-compose.yml` đã được chuẩn bị sẵn trong repository dưới dạng cấu hình mở rộng dự phòng. Trong thiết kế scale-out, Apache Kafka có thể đóng vai trò là **Message Queue** đặt giữa các Spark job xử lý batch và các inference worker, cho phép bổ sung thêm GPU worker theo chiều ngang (horizontal scaling) mà không cần thay đổi kiến trúc tổng thể của hệ thống.
...
*Tài liệu được biên soạn và xác minh trực tiếp từ source code của repository `smart-checkout` — VSSCS project. Tất cả thông số kỹ thuật bao gồm cổng kết nối (port), ngưỡng similarity (threshold), kích thước batch, tên model, tên collection, số chiều embedding và cấu trúc đường dẫn đều được lấy từ code thực tế, không suy đoán.*
